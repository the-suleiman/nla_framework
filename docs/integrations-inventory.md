...
| **Phone auth** | `Config.Auth.ByPhone` | [`templates/project.go`](../templates/project.go) conditional SQL + Vue + `phone.go` | Email auth stays default when phone is off (`Start` in [`main.go`](../main.go)); SMS response reads use `io.ReadAll`. |
